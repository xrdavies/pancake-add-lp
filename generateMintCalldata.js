...
async function generateMintData() {
    ...
    function getTickSpacing(feeAmount) {
        switch (feeAmount) {
            ...
            case 2500:  return 50;   // 0.25% (PancakeSwap uses 50 for this tier on BSC)
    ...
    if (priceObject) {
        ...
        if (priceObject.scalar && priceObject.scalar.numerator && priceObject.scalar.denominator) {
            console.log(`  priceObject.scalar (Fraction): ${priceObject.scalar.numerator.toString()}/${priceObject.scalar.denominator.toString()}`);
        }
    } else {
    ...
    try {
        ...
        if (tickLowerForCalc >= tickUpperForCalc) {
            throw new Error(`Adjusted tickLower (${tickLowerForCalc}) must be less than adjusted tickUpper (${tickUpperForCalc}).`);
        }
        ...
        console.log('  - The pool price is exactly at or outside one of the tick boundaries in a way that results in zero liquidity for one asset.');
    }
    ...
    console.log(`Ticks used for SDK calculation (after adjustment to be multiples of ${tickSpacing}): Lower=${tickLowerForCalc}, Upper=${tickUpperForCalc}`);
    if (calculationError) {
    ...
    console.log(`Demo amount1Desired (from transaction, used for actual encoding): ${mintParams.amount1Desired.toString()}`);
    console.log('Note: The SDK-calculated amount1Desired will likely differ from the demo amount1Desired unless the assumed current pool price, token decimals, and tick adjustments precisely match the historical conditions of the demo transaction.');
    console.log('The script will continue to use the original demo amount1Desired for ABI encoding to ensure verification.');
    console.log('--------------------------------------------------\n');
    ...
    // --- 3. Verification ---
    const originalRawData = '';
